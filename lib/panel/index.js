...
function normalizeExperienceForPanel(experience) {
  return {
    ...
    canonical_terms: [...(experience.canonical_terms || [])],
    suggested_canonical_terms: [...(experience.suggested_canonical_terms || [])],
    ...
    superseded_by_ids: [...(experience.superseded_by_ids || [])],
    supersedes_ids: [...(experience.supersedes_ids || [])]
  };
}

function normalizePaperForPanel(paper) {
  return {
    id: paper.id,
    title: paper.title,
    ...
    canonical_terms: [...(paper.canonical_terms || [])],
    suggested_canonical_terms: [...(paper.suggested_canonical_terms || [])],
    venue: paper.venue || "",
...
function inferNodeType(nodeId) {
  ...
  if (/^P\d+/u.test(nodeId)) {
    return "paper";
  }

  const prefix = String(nodeId || "").split(":")[0];
  if (["tag", "tech", "file", "concept", "topic", "keyword", "author", "venue", "canonical"].includes(prefix)) {
    return prefix;
  }
...
function buildPanelViewModel(runtime) {
  ...
  const experiences = getExperiences(index)
    .map((experience) => normalizeExperienceForPanel({
      ...experience,
      superseded_by_ids: evolution.supersededBy.get(experience.id) || [],
      supersedes_ids: evolution.supersedes.get(experience.id) || []
    }));
  const papers = getPapers(index).map(normalizePaperForPanel);
  ...
  return {
    ...
    recent_papers: pickRecentExperiences(papers),
    all_papers: papers,
...
function renderExperienceCard(experience) {
  ...
  const searchText = [
    ...
    experience.fix,
    experience.scope,
    ...(experience.superseded_by_ids || []),
    ...(experience.supersedes_ids || []),
    ...(experience.tags || []),
    ...(experience.techs || []),
  ...
  return [
    ...
    (experience.superseded_by_ids || []).length
      ? `  <div class="meta-line evolution-line"><span>Recommended current version: ${escapeHtml(experience.superseded_by_ids.join(", "))}</span></div>`
      : "",
    (experience.supersedes_ids || []).length
      ? `  <div class="meta-line evolution-line"><span>Supersedes: ${escapeHtml(experience.supersedes_ids.join(", "))}</span></div>`
      : "",
    "  <div class=\"meta-line\">",
...
function renderPaperCard(paper) {
  const topics = (paper.topics || []).map((topic) => renderPill(topic, "tag")).join("");
  const keywords = (paper.keywords || []).slice(0, 4).map((keyword) => renderPill(keyword, "tech")).join("");
  const metaBits = [
    paper.venue || "n/a",
    paper.year || "n/a"
  ];
  ...
  return [
    `<article class="experience-card paper-card" data-paper-id="${escapeHtml(paper.id)}">`,
    "  <div class=\"experience-head\">",
    `    <h3>${escapeHtml(paper.id)} 路 ${escapeHtml(paper.title)}</h3>`,
    "    <div class=\"experience-badges\">",
    `      ${renderPill(paper.status || "ACTIVE", `status-${String(paper.status || "").toLowerCase()}`)}`,
    "    </div>",
    "  </div>",
    `  <p class="problem">${escapeHtml(paper.summary || paper.abstract)}</p>`,
    `  <p class="solution">${escapeHtml((paper.authors || []).join(", ") || "Unknown authors")}</p>`,
    "  <div class=\"meta-line\">",
    `    <span>${escapeHtml(metaBits.join(" / "))}</span>`,
    `    <span>Updated: ${escapeHtml(formatDate(paper.updated_at || paper.created_at))}</span>`,
    "  </div>",
    topics || keywords ? `  <div class="pill-row">${topics}${keywords}</div>` : "",
    paper.url ? `  <div class="file-row"><code>${escapeHtml(paper.url)}</code></div>` : "",
    "</article>"
  ].filter(Boolean).join("\n");
}
...
function renderPapers(papers) {
  if (!papers.length) {
    return "<div class=\"empty\">No research papers yet.</div>";
  }

  return papers.map(renderPaperCard).join("\n");
}
...
function renderCandidates(candidates) {
  ...
  return candidates.map((candidate) => [
    `<article class="candidate-card" data-candidate-id="${escapeHtml(candidate.id)}" data-entry-kind="${escapeHtml(String(candidate.entry_kind || "Experience").toLowerCase())}" data-risk-level="${escapeHtml((((candidate.review_gate || {}).riskLevel) || "unknown").toLowerCase())}" data-file-haystack="${escapeHtml((((candidate.anchors || {}).files) || []).join(" ").toLowerCase())}">`,
    `  <strong>${escapeHtml(candidate.id)} · ${escapeHtml(candidate.title)}</strong>`,
    `  <div>${renderPill(candidate.entry_kind || "Experience")} ${renderPill(candidate.status || "UNKNOWN")} ${renderPill(candidate.confidence || "UNKNOWN")}</div>`,
...
function renderReviewWorkspace(candidates) {
  ...
  return [
    ...
    "      <div class=\"segmented-control\" role=\"tablist\" aria-label=\"Candidate type filter\">",
    "        <button class=\"segment-button active\" type=\"button\" data-entry-filter=\"all\">All</button>",
    "        <button class=\"segment-button\" type=\"button\" data-entry-filter=\"experience\">Experience</button>",
    "        <button class=\"segment-button\" type=\"button\" data-entry-filter=\"paper\">Paper</button>",
    "      </div>",
    ...
    "  <div class=\"card\">",
    "    <h2>Unified Candidate Queue</h2>",
    "    <p class=\"subheading\">Review paper and experience candidates in one queue, using the type label on each card to distinguish what will be accepted into the graph.</p>",
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .status-superseded { background: #e5e7eb; color: #475569; }
    .evolution-line { color: #475569; font-size: 12px; }
  ...
  <main>
    ...
    <section id="overviewPanel" class="tab-panel active">
      <section class="grid metrics">
        ...
        ${renderMetricCard("Papers", stats.paper_count || 0, "Tracked research papers")}
      ...
      <section class="grid two-col" style="margin-top:16px;">
        <div class="card">
          ...
          <p class="subheading">Keep a lightweight map of the papers, topics, and venues behind a direction before you start implementation work.</p>
          <div class="experience-list">
            ${renderPapers(view.recent_papers)}
          </div>
        </div>
        <aside class="grid">
          <div class="card">
            <h2>Top Topics</h2>
            ${renderCountBars(countIndexBucket((view.indexes || {}).by_topic || {}), "No paper topics yet.")}
          </div>
          <div class="card">
            <h2>Top Venues</h2>
            ${renderCountBars(countIndexBucket((view.indexes || {}).by_venue || {}), "No paper venues yet.", "graph-fill")}
          </div>
        </aside>
      </section>
...
node scripts/ekg.js capture-status</pre>
  ...
  <script>
    ...
    const reviewTypeButtons = Array.from(document.querySelectorAll("[data-entry-filter]"));
    ...
    function openDetails(experienceId) {
      ...
      drawerBody.innerHTML = [
        ...
        (experience.superseded_by_ids || []).length ? '<section class="detail-section"><h4>Recommended Current Version</h4>' + renderCodeList(experience.superseded_by_ids || []) + '</section>' : '',
        (experience.supersedes_ids || []).length ? '<section class="detail-section"><h4>Supersedes</h4>' + renderCodeList(experience.supersedes_ids || []) + '</section>' : '',
    ...
    function updateReviewFilters() {
      ...
      const entryFilter = String(activeEntryFilter || 'all').toLowerCase();
      let visibleCount = 0;
      ...
      reviewCards.forEach((card) => {
        ...
        const entryKind = String(card.getAttribute('data-entry-kind') || 'experience').toLowerCase();
        const matchRisk = !highRiskOnly || riskLevel === 'high';
        const matchFile = !fileQuery || fileHaystack.includes(fileQuery);
        const matchEntry = entryFilter === 'all' || entryKind === entryFilter;
        const visible = matchRisk && matchFile && matchEntry;
    ...
    function setReviewEntryFilter(nextFilter) {
      activeEntryFilter = String(nextFilter || 'all').toLowerCase();
      reviewTypeButtons.forEach((button) => {
        const buttonFilter = String(button.getAttribute('data-entry-filter') || 'all').toLowerCase();
        button.classList.toggle('active', buttonFilter === activeEntryFilter);
        button.setAttribute('aria-pressed', buttonFilter === activeEntryFilter ? 'true' : 'false');
      });
      updateReviewFilters();
    }
    ...
    function initGraphView() {
      ...
      cy = window.cytoscape({
        ...
        style: [
          ...
          { selector: 'node[nodeType = "paper"]', style: { 'background-color': '#fde68a', color: '#111827', 'font-weight': 700, 'border-color': '#d97706' } },
          ...
          { selector: 'node[nodeType = "topic"]', style: { 'background-color': '#fed7aa', color: '#111827', 'border-color': '#ea580c' } },
          { selector: 'node[nodeType = "author"]', style: { 'background-color': '#fbcfe8', color: '#111827', 'border-color': '#db2777' } },
          { selector: 'node[nodeType = "venue"]', style: { 'background-color': '#c7d2fe', color: '#111827', 'border-color': '#4f46e5' } },
          { selector: 'node[nodeType = "canonical"]', style: { 'background-color': '#d1fae5', color: '#111827', 'border-color': '#059669' } },
          {
            selector: 'edge',
            style: {
              width: 1.4,
    ...
    reviewTypeButtons.forEach((button) => {
      button.addEventListener('click', () => {
        setReviewEntryFilter(button.getAttribute('data-entry-filter') || 'all');
      });
    });
    setReviewEntryFilter(activeEntryFilter);
