...
function createCaptureCandidate(state, input = {}, options = {}) {
  ...
  if (duplicate) {
    ...
    duplicate.symptom = firstNonEmpty(duplicate.symptom, input.symptom);
    duplicate.cause = firstNonEmpty(duplicate.cause, input.cause);
    duplicate.fix = firstNonEmpty(duplicate.fix, input.fix);
    duplicate.scope = firstNonEmpty(duplicate.scope, input.scope);
    ...
    duplicate.entry_kind = firstNonEmpty(input.entry_kind, duplicate.entry_kind);
    if (input.entry_payload && typeof input.entry_payload === "object") {
      duplicate.entry_payload = {
        ...((duplicate.entry_payload && typeof duplicate.entry_payload === "object") ? duplicate.entry_payload : {}),
        ...input.entry_payload
      };
    }
    duplicate.origin = {
      ...
      task_full: firstNonEmpty(input.task, (duplicate.origin || {}).task_full),
      summary: truncate(firstNonEmpty(input.summary, (duplicate.origin || {}).summary), 240),
      summary_full: firstNonEmpty(input.summary, (duplicate.origin || {}).summary_full)
    };
  ...
  const candidate = {
    ...
    symptom: firstNonEmpty(input.symptom),
    cause: firstNonEmpty(input.cause),
    fix: firstNonEmpty(input.fix),
    scope: firstNonEmpty(input.scope),
    ...
    entry_kind: firstNonEmpty(input.entry_kind),
    entry_payload: input.entry_payload && typeof input.entry_payload === "object"
      ? { ...input.entry_payload }
      : undefined,
    writer: input.writer || {},
    anchors: {
    ...
    origin: {
      ...
      task_full: firstNonEmpty(input.task),
      summary: truncate(firstNonEmpty(input.summary), 240),
      summary_full: firstNonEmpty(input.summary)
    },
    created_at: now,
    updated_at: now
  };
