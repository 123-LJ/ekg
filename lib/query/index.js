...
function buildExperienceEvolutionMaps(index) {
    ...
    (experience.relations || [])
      ...
      .forEach((relation) => {
        if (relation.type === "supersedes") {
          outgoing.push(relation.target);
          supersededBy.set(relation.target, unique([...(supersededBy.get(relation.target) || []), experience.id]));
        }
        if (relation.type === "deprecated-by") {
          supersededBy.set(experience.id, unique([...(supersededBy.get(experience.id) || []), relation.target]));
        }
      });

    if (outgoing.length) {
      supersedes.set(experience.id, unique(outgoing));
    }
  });
...
function queryExperiences(index, query, limit = 5) {
  ...
  const includeSuperseded = Boolean(query.includeSuperseded);
  return getExperiences(index)
    ...
    .map((experience) => {
      ...
      const supersededPenalty = !includeSuperseded && supersededBy.length ? 6 : 0;
      const activeSupersedeBonus = supersedes.length ? Math.min(4, supersedes.length * 2) : 0;
      const statusPenalty = experience.status === "SUPERSEDED" && !includeSuperseded ? 10 : 0;
      const score = lexical.score + semanticBoost + activeSupersedeBonus - supersededPenalty - statusPenalty;
      const reasons = [
        ...lexical.reasons,
      ...
      if (supersedes.length) {
        reasons.push(`supersedes: ${supersedes.join(", ")}`);
      }
      if (experience.status === "SUPERSEDED") {
      ...
      return {
        ...
        score,
        reasons: unique(reasons),
        superseded_by_ids: supersededBy,
