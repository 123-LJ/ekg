...
function mapOpenAlexWorkToPaper(work, options = {}) {
  ...
  const ids = work.ids || {};
  const abstract = work.abstract || work.display_name || "";
  const paper = {
    title: work.display_name || "",
    abstract,
    summary: buildCompactText(work.summary || abstract || work.display_name || "", {
      maxLength: 240,
      maxSegments: 3
    }),
    findings: "",
...
function mapSemanticScholarPaperToPaper(paper, options = {}) {
  ...
  const nextPaper = {
    title: paper.title || "",
    abstract,
    summary: buildCompactText(paper.summary || abstract || paper.title || "", {
      maxLength: 240,
      maxSegments: 3
    }),
    findings: "",
