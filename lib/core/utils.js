...
function truncateText(value, maxLength = 240) {
  ...
  const slice = normalized.slice(0, Math.max(0, maxLength - 3)).trim();
  return `${slice}...`;
}

function buildCompactText(value, options = {}) {
  const maxLength = Math.max(40, Number(options.maxLength || 220));
  const maxSegments = Math.max(1, Number(options.maxSegments || 3));
  const normalized = normalizeWhitespace(value);
  if (!normalized) {
    return "";
  }
  ...
  const segments = splitTextSegments(normalized);
  if (!segments.length) {
  ...
  for (const segment of segments) {
    ...
    if (!clean || compact.includes(clean)) {
      continue;
    }

    const separator = compact.length ? " " : "";
    const candidate = `${compact.join(" ")}${separator}${clean}`.trim();
  ...
  const result = compact.join(" ").trim();
  return result.length <= maxLength ? result : truncateText(result, maxLength);
}
